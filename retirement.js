...
//set data
	constructor.prototype = {
		...
		getAnnualSavings: function(){
			var yearsTilRetire = this.get("retirementAge") - this.get("currentAge");
			var annualSavingsArray = [];
			var savings = this.getPeriodicSavings();

			for(var i = 0; i < yearsTilRetire; i++) {
				annualSavingsArray.push(savings*this.get("compounding"));
			}
			return annualSavingsArray;
		},
...
var RetirementCalc = (function() {
	...
	constructor.prototype = {
		...
		getLineChartData: function(){
			...
			var retirementPlan = this.userData.getFullRetirementBalancesPlan();
			console.log(retirementAmounts, retirementPlan)
			var self = this;
			var ageLabel;
			var goalIncome = 0;
			var socialSecurity = 0;
			...
			_(zip(retirementAmounts, retirementPlan)).forEach(function(n){
...
function GetUrlParam(name) { 
	...
	if (results && results.length>0) return Number(results[1]) || 0;
	return null
}
